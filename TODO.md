...
| —                                                                         | Multi-device sync (v2)             | Specced end-to-end in `specs/sync.md`. **Phases 0–5 are shipped**: SWA auth, Cosmos, `seq`-cursor pull/push, last-write-wins merge, local outbox and the live `CloudSyncEngine`. Remaining: phase 6 (photo bytes to Blob Storage + per-user quota), phase 7 (sync UI and server-side delete), phase 8 (hardening, two-device E2E). |
