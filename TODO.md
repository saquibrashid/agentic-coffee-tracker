...
| —                                                                         | Multi-device sync (v2)             | Specced end-to-end in `specs/sync.md`. **All phases (0–8) are shipped**: SWA auth, Cosmos, `seq`-cursor pull/push, last-write-wins merge, local outbox, the live `CloudSyncEngine`, photo bytes to Blob Storage with a 500 MB per-user quota, the Settings sync UI with server-side delete, and hardening — a per-user rate limit, a 20,000-record quota, cross-account isolation tests, and a two-device Playwright test running in CI. Sync is **live**: `SYNC_ACCESS_MODE=owner` with the owner account in `SYNC_ALLOWLIST`. Adding people later is a config change, not a code change — see `docs/deployment.md` -> Access. |
...
| [#109](https://github.com/saquibrashid/agentic-coffee-tracker/issues/109) | Decaf filter                       | **Shipped** via [#277](https://github.com/saquibrashid/agentic-coffee-tracker/issues/277). `CoffeeBean.caffeine` now exists as an optional four-value enum, with a capture-form control, an extraction-prompt and JSON-schema change, a name-based inference for the many bags that only say "Decaf" in the title, and a Caffeine filter in the library.                                                                                                                                                                                                                                                                        |
