 ...
 * hold back a single top-marks rating without ever letting volume decide the
 * order: shrinkage can only pull a value *toward* the mean, never past it, so
 * something the user scores below their own average can never outrank something
 * they score above it, however often they drink it.
 *
 ...
 * one: a note averaging 6.5 across 8 ratings outranked one averaging 9.0 across
 * 2, on a screen whose heading is "Your taste map" (issue #199). Using a raw
 * average on a 1–10 scale made it worse, because every rating then contributes
...
/**
 * A value must have at least this many ratings before it can be ranked.
 *
 * One is enough because shrinkage, not this threshold, is what stops a lone
 * rating dominating: a single rating is pulled most of the way back to the
 * baseline.
 */
 ...
 *
 * Lives in `services/ratings/shrink` because Analytics ranks the same values
 * from the same ratings and has to agree with this screen (issue #202).
 */
  ...
  return Array.from(map.entries())
    ...
    .map(([value, acc]) => {
      ...
      return {
        ...
        weightedScore: shrinkToBaseline(acc.count, averageScore, baseline),
      };
    })
