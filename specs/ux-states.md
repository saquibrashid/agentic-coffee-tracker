...
- The verdict is arithmetic on the user's own ratings, not a model call. It works offline, costs
  ...
  a real count and average ("Coffees from Ethiopia average 9.2/10 across 7 ratings"). The unit is
  deliberately "ratings", not "cups": the number counts rating records, and one bag rated twice is
  two of them however many cups were actually poured from it.
...
- The estimate is shrunk toward the user's own average in proportion to how much evidence there is,
  so a single top-marks rating cannot produce a confident rave.
